...
import {TeamStats,ProjectStats,DocumentTypeCounts,DashboardStats,User} from '../../../../types'
...
const STAT_KEYS = {
  ...
  projects: ['active', 'inactive', 'total'] as const,   // map “finished / inProgress …” → active if you wish
} as const;
...
const Colors: Record<TeamKey | ProjectKey, string> = {
 ...
 const fetch= async()=> {
        ...
        <DocumentBarChart  data={documentsCount ? [{ name: "Documents", ...documentsCount }] : []} />
           ...
           {teamsData && STAT_KEYS.teams.map((key) => (
            ...
            >
              ...
              <div style={{ maxHeight: 220, height: 220, overflowY:'auto' }}>
              {projectsData && STAT_KEYS.projects.map((key) => (
              <div key={key} style={{ marginLeft: 20, marginBottom: 8 }}>
                <h3 style={{ margin: 0, textTransform:'capitalize' }}>{key}</h3>
  ...
  renderItem={(item) => (
    ...
    >
      <List.Item.Meta
        avatar={<img src={item.profilePicture}  style={{ borderRadius: '50%', width: 40, height: 40 }} />}
        title={
