...
const Register = async (req:Request, res: Response) => {
    try {
        ...
        const emailCheckParams = {
          ...
          const token = jwt.sign({ id: validatedUser.user_id }, process.env.JWT_SECRET, { expiresIn: '2d' });
...
const Login = async (req:Request, res: Response) => {
    try {
        ...
        if (!process.env.JWT_SECRET) {
          ...
          const token = jwt.sign({ id: user.user_id }, process.env.JWT_SECRET, { expiresIn: '2d' });
    ...
    try {
        ...
        const idCheckParams = {
          ...
          const updateParams: UpdateCommandInput = {
            ...
            UpdateExpression: 'SET isAccountVerified = :verified, verifyOTP = :emptyOtp, verifyOTPExpiredAt = :zero',
            ExpressionAttributeValues: {
              ':verified': true,
              ':emptyOtp': '',
              ':zero': 0,
...
export { Login, Register, logout, sendVerifyOtp, verifiedEmail, isAuthenticated, sendResetOtp,resetPassword,getUserById };